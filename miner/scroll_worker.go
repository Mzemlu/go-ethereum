...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *worker {
	...
	worker.asyncChecker = ccc.NewAsyncChecker(worker.chain, runtime.GOMAXPROCS(0), false).WithOnFailingBlock(worker.onBlockFailingCCC)
...
func (w *worker) newWork(now time.Time, parentHash common.Hash, reorgReason error) error {
	...
	cccLogger := ccc.NewLogger(types.RowConsumptionLimit)
	vmConfig.Debug = true
